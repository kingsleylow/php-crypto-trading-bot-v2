  ...
  public function run() {
    ...
    while(true) {
      ...
      foreach ($read as $socket) {
        ...
        $this->sendMessagesfromBot();
        $cmnds->checkAndexecNewCommands(); 
        
        
    }
  }
    private function sendMessagesfromBot(){
        /*if(file_exists("msgs_to_web.json")){
            foreach($this->users as $user){
                $this->send($user,file_get_contents("msgs_to_web.json"));
            }   
            unlink("msgs_to_web.json");
        }*/
        $msgs = glob('msgs_to_web/*.{msg}', GLOB_BRACE);
        foreach($msgs as $msg) {
            foreach($this->users as $user){
                $this->send($user,file_get_contents($msg));
            }   
            unlink($msg);
        }
